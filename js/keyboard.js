...
var next=0;
	...
	function showKeybord(){
			...
			for(var i=0;i<keyNums.length;i++){
				if(i>=0 && i<=9){
                    // console.log($('ikey'+i));
                    sss=sss+'<li class="disablednum ikey ikey'+i+' '+(i>9?"li_zm":"li_num")+' '+(i>28?"li_w":"")+'" ><span onclick="choosekey(this,'+i+');">'+keyNums[i]+'</span></li>';
                }
                else if(i>36 && i<41){
                    // console.log($('ikey'+i));
                    sss=sss+'<li class="disabled ikey ikey'+i+' '+(i>9?"li_zm":"li_num")+' '+(i>28?"li_w":"")+'" ><span onclick="choosekey(this,'+i+');">'+keyNums[i]+'</span></li>';
                }
                    else{
			...
			$("#pro").html("<ul class='clearfix ul_keybord'>"+sss+"<li class='li_close' onclick='closePro();'><span>关闭</span></li></ul>");
	}
	...
	function showAbc(next){
		if(next!=0){
			$('.disablednum').removeClass('disablednum');
		}
		else if(next==0){
            $('.ikey0').addClass('disablednum');
            $('.ikey1').addClass('disablednum');
            $('.ikey2').addClass('disablednum');
            $('.ikey3').addClass('disablednum');
            $('.ikey4').addClass('disablednum');
            $('.ikey5').addClass('disablednum');
            $('.ikey6').addClass('disablednum');
            $('.ikey7').addClass('disablednum');
            $('.ikey8').addClass('disablednum');
            $('.ikey9').addClass('disablednum');
           }
		...
		if(next==6){
            ...
            $('.ikey37').addClass('disabled');
            $('.ikey38').addClass('disabled');
            $('.ikey39').addClass('disabled');
            $('.ikey40').addClass('disabled');            
        	}
	...
	function choosekey(obj,jj){
		if(jj==29){
			...
			for(var i = 0; i<$(".input_pp").length;i++){
			  ...
			  if( $(".input_pp:eq("+ next +")").hasClass("hid") && !$(".input_pp:eq("+ next+")").hasClass("show") ){
					return;
			  }
